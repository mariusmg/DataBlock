...
description: MySql QueryCriteria implementation
...
namespace voidsoft.DataBlock
{
	...
	/// </summary>
	internal class MySqlQueryCriteriaGenerator : IQueryCriteriaGenerator
	{
		//consts
		...
		internal ExecutionQuery GenerateWithJoin(QueryCriteria criteria)
		{
			...
			try
			{
				...
				execQuery = SqlGenerator.GenerateSelectQuery(DatabaseServer.MySQL, criteria);
				...
				for (int i = 0; i < criteria.JoinCriteriaConditions.Length; i++)
				{
					...
					sbuild.Append(SqlGenerator.GetTableName(DatabaseServer.MySQL, criteria.JoinCriteriaConditions[i].ForeignKeyFieldTableName) + " ON " +
					              SqlGenerator.GetTableName(DatabaseServer.MySQL, criteria.JoinCriteriaConditions[i].PrimaryKeyFieldTableName) + "." +
					              criteria.JoinCriteriaConditions[i].PrimaryKey.fieldName + "=" +
					              SqlGenerator.GetTableName(DatabaseServer.MySQL, criteria.JoinCriteriaConditions[i].Criteria.TableName) + "." +
		...
		internal ExecutionQuery GenerateWithoutJoin(QueryCriteria criteria)
		{
			...
			try
			{
				...
				{
					execQuery = SqlGenerator.GenerateUpdateQuery(DatabaseServer.MySQL, criteria.TableName, criteria.Fields, false);
				}
				else if (generatorType == QueryCriteriaGeneratorType.Delete)
				{
					execQuery = SqlGenerator.GenerateDeleteQuery(DatabaseServer.MySQL, criteria.TableName);
				}
				...
				for (int i = 0; i < conditions.Length; i++)
				{
					...
					switch (conditions[i].CriteriaOperator)
					{
						case CriteriaOperator.Between:
							...
							IDataParameter paramBetweenFirst = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							IDataParameter paramBetweenSecond = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							...
							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + conditions[i].Field.fieldName + " BETWEEN " +
						...
						case CriteriaOperator.Different:
							...
							IDataParameter paramDifferent = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							listParameters.Add(paramDifferent);
							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + "<>" + isql.GetValue(paramDifferent));
							break;

						case CriteriaOperator.Like:
							field.fieldValue = "%" + conditions[i].Values[0] + "%";

							IDataParameter paramLike = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							listParameters.Add(paramLike);

							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " LIKE " + isql.GetValue(paramLike));
							break;
						...
						case CriteriaOperator.Equality:
							...
							IDataParameter paramEquality = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							listParameters.Add(paramEquality);

							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + "=" + isql.GetValue(paramEquality));
							break;

						case CriteriaOperator.IsNull:
							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " is null");
							break;

						case CriteriaOperator.IsNotNull:
							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " is not null");
							break;
						...
						case CriteriaOperator.Smaller:
							...
							IDataParameter paramSmaller = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							listParameters.Add(paramSmaller);

							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " < " + isql.GetValue(paramSmaller));
							break;

						case CriteriaOperator.SmallerOrEqual:
							field.fieldValue = conditions[i].Values[0];

							IDataParameter paramSmallerOrEqual = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							listParameters.Add(paramSmallerOrEqual);

							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " <= " + isql.GetValue(paramSmallerOrEqual));
							break;

						case CriteriaOperator.Higher:
							field.fieldValue = conditions[i].Values[0];

							IDataParameter paramHigher = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							listParameters.Add(paramHigher);

							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " > " + isql.GetValue(paramHigher));
							break;

						case CriteriaOperator.HigherOrEqual:
							field.fieldValue = conditions[i].Values[0];

							IDataParameter paramHigherOrEqual = DataConvertor.ConvertToDataParameter(DatabaseServer.MySQL, tableName, field, ref listParameterNames);
							listParameters.Add(paramHigherOrEqual);

							sbuild.Append(" " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " >= " + isql.GetValue(paramHigherOrEqual));
							break;
						...
						case CriteriaOperator.OrderBy:
							if (sbOrderByCriteria.Length == 0)
							{
								...
								sbOrderByCriteria.Append("ORDER BY " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " " +
								                         conditions[i].Values[0]);
							}
							else
							{
								//add "," for the subsequent criterias
								sbOrderByCriteria.Append(", " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + field.fieldName + " " + conditions[i].Values[0]);
							}
							break;
						...
						case CriteriaOperator.Distinct:
							...
							//get the field
							fieldName = SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + conditions[i].Field.fieldName;
							...
							tempString = tempString.Insert(SELECT_FIELD_LENGTH,
							                               " distinct " + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + conditions[i].Field.fieldName);
						...
						case CriteriaOperator.Max:
							...
							//add it at the beginning of the select
							tempString = tempString.Insert(SELECT_FIELD_LENGTH,
							                               " max(" + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + conditions[i].Field.fieldName + ")");
						...
						case CriteriaOperator.Min:
							...
							//add it at the beginning of the select
							tempString = tempString.Insert(SELECT_FIELD_LENGTH,
							                               " min(" + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + conditions[i].Field.fieldName + ")");
						...
						case CriteriaOperator.Count:
							...
							//add it at the beginning of the select
							tempString = tempString.Insert(SELECT_FIELD_LENGTH,
							                               " count(" + SqlGenerator.GetTableName(DatabaseServer.MySQL, tableName) + "." + conditions[i].Field.fieldName + ")");
